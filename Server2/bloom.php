...
class BloomFilter {
	...
	{
		$con=mysqli_connect("139.59.15.153", "root", "password");
		...
		//Adding  the paper id in the database
		$sql="insert into filters(id) values(".($no_of_rows+1).");";

		if(mysqli_query($con,$sql))
		{
			//echo "Paper id added successfully"; 
			$x1=0;
		}
		else 
			echo "Paper id not added ";

		
		$sql="update filters set arr='".$this->bitField."' where id=".($no_of_rows+1).";";
		...
		{
			echo " Filter addition failed";	
			echo "    ".mysqli_error($con);
		}
		return ($no_of_rows+1);
	}
	public function extractFromDB($varr)
	{
		$con=mysqli_connect("139.59.15.153", "root", "password");
		mysqli_select_db($con,"project_major");
		$result = mysqli_query($con,"select * from filters");
		...
		foreach($varr as $value)
		{
			...
			{
				$sql="select * from filters where id=".($i+1).";";
				$result=mysqli_query($con,$sql);
				$row=$result->fetch_assoc();
				$this->bitField=$row["arr"];
				$score[$i]=$score[$i]+$this->maybeInSet($value);
			}
		}
